...
export default function GaleriaPublica({ fotos }: { fotos: Post[] }) {
  const [indice, setIndice] = useState<number | null>(null);

  const cerrar = useCallback(() => setIndice(null), []);
  const anterior = useCallback(() =>
    setIndice((i) => (i !== null ? (i - 1 + fotos.length) % fotos.length : null)), [fotos.length]);
  const siguiente = useCallback(() =>
    setIndice((i) => (i !== null ? (i + 1) % fotos.length : null)), [fotos.length]);
