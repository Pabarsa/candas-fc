...
export default function Navbar() {
  ...
  useEffect(() => { setOpen(false); setClubOpen(false); }, [pathname]);
  ...
  const clubLinks = [
    { href: "/equipo",    label: "Plantilla" },
    { href: "/veteranos", label: "Veteranos" },
  ];

  const isClubActive = ["/equipo", "/veteranos"].includes(pathname);
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <div ref={clubRef} className="relative">
              <button
                ...
                className={`flex items-center gap-1 text-sm font-medium transition-colors duration-200 [text-shadow:0_1px_4px_rgba(0,0,0,0.8)] ${
                  isClubActive ? "text-white" : "text-white/80 hover:text-white"
                }`}
              >
                Club
                <svg className={`w-3 h-3 transition-transform duration-200 ${clubOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {clubOpen && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-36 bg-black/95 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-xl z-50">
                  {clubLinks.map(({ href, label }) => (
                    <Link key={href} href={href}
                      className={`block px-4 py-2.5 text-sm transition-colors duration-150 ${
                        pathname === href ? "text-white bg-white/5" : "text-white/70 hover:text-white hover:bg-white/5"
                      }`}>
                      {label}
                    </Link>
                  ))}
      ...
      {open && (
        <div className="fixed inset-0 z-40 bg-black/98 backdrop-blur-md flex flex-col pt-14">
          <nav className="flex-1 overflow-y-auto px-6 py-8 flex flex-col gap-1">
            ...
            {/* Club links en móvil */}
            <p className="text-white/20 text-xs uppercase tracking-widest mt-4 mb-1">Club</p>
            {clubLinks.map(({ href, label }) => (
              <Link key={href} href={href}
                className={`py-3.5 text-lg font-semibold border-b border-white/5 transition-colors duration-200 ${
                  pathname === href ? "text-white" : "text-white/40"
